...
const tempDir = await mkdtemp(path.join(tmpdir(), 'outreach-v2-test-'));
const failures = [];
...
function runNpm(script, args = [], expectedSuccess = true) {
...
function bw({ domain, name, country = 'US', revenue = 90000, employees = 12, spend = 900, sku = 120, email }) {
  return {
    D: domain,
    R: revenue,
    E: employees,
...
try {
  ...
  // Discovery: input exports must get the same hard commercial filters as API results.
  const valid = Array.from({ length: 10 }, (_, i) => bw({
  ...
  check(discovered.prospects.length === 10, `discovery should return 10 valid new records, got ${discovered.prospects.length}`);
  check(!discovered.prospects.some((p) => p.domain === 'email-duplicate.example'), 'a BuiltWith email matching the permanent ledger should cause duplicate exclusion');
  check(!discovered.prospects.some((p) => ['low-revenue.example', 'low-employees.example', 'low-spend.example', 'low-sku.example', 'wrong-country.example'].includes(p.domain)), 'commercial/country filters should be enforced on imported data');

  // Thin qualification cannot pass without the downstream stages and full fields.
  const thin = Array.from({ length: 25 }, (_, i) => ({
  ...
  await writeJson(path.join(campaignDir, '02-qualified.json'), { prospects: thin });
  runNpm('outreach:validate', [`--week=${week}`], false);

  const qs = Array.from({ length: 25 }, (_, i) => qualified(i));
  ...
  await writeFile(
    ...
    '# Campaign Email Standard\n\nThe subject stays short and factual. The opening uses current account research. Initial copy is concise and prospect-specific. Every follow-up adds a different commercial angle. The CTA asks directly whether the demonstrated implementation should be built into the live store. Compliance is checked per jurisdiction and deliverability rules are confirmed before provider loading.\n\nResearch sources:\nhttps://www.gong.io/\nhttps://www.ftc.gov/\n',
    'utf8'
  );
  await writeJson(path.join(campaignDir, '06-sequences.json'), { sequences: ss });

  // Scheduling must not be possible before preflight.
  runNpm('outreach:schedule', [`--week=${week}`], false);

  // Generic/evidence-free sequence changes must be caught.
  const broken = structuredClone(ss);
  broken[0].touches[1].evidence_used = [];
  broken[0].touches[1].purpose = broken[0].touches[0].purpose;
  await writeJson(path.join(campaignDir, '06-sequences.json'), { sequences: broken });
  runNpm('outreach:validate', [`--week=${week}`], false);

  // Valid full campaign should preflight successfully.
  await writeJson(path.join(campaignDir, '06-sequences.json'), { sequences: ss });
  runNpm('outreach:validate', [`--week=${week}`], true);
  check(await readFile(path.join(campaignDir, 'preflight.json'), 'utf8').then(Boolean), 'successful validation should create preflight.json');

  // Any stage change after preflight must invalidate scheduling.
  const mutated = structuredClone(ss);
  mutated[0].touches[0].body_text += ' Changed after validation.';
  await writeJson(path.join(campaignDir, '06-sequences.json'), { sequences: mutated });
  runNpm('outreach:schedule', [`--week=${week}`], false);

  // Restore, preflight and schedule.
  await writeJson(path.join(campaignDir, '06-sequences.json'), { sequences: ss });
  runNpm('outreach:validate', [`--week=${week}`], true);
  ...
  // Ledger sync must persist both selected and rejected serious evaluations.
  runNpm('outreach:ledger-sync', [`--week=${week}`], true);
  const ledger = JSON.parse(await readFile(ledgerFile, 'utf8'));
  check(ledger.entries.some((entry) => entry.domain === 'qualified-0.example' && entry.status === 'qualified'), 'ledger should persist qualified prospects');
  check(ledger.entries.some((entry) => entry.domain === 'rejected-no-email.example' && entry.status === 'rejected' && entry.rejection_reason === 'no_verified_email'), 'ledger should persist rejected candidates with rejection reason');

  if (failures.length) {
    console.error(`Outreach system regression suite failed with ${failures.length} issue(s):`);
    for (const failure of failures) console.error(`- ${failure}`);
    process.exit(1);
  }

  console.log('Outreach system end-to-end regression suite passed.');
} finally {
