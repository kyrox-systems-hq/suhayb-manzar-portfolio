...
const tempDir = await mkdtemp(path.join(tmpdir(), 'outreach-v4-test-'));
const failures = [];
...
try {
  ...
  await runNpm('outreach:schedule', [`--week=${week}`], true);
  ...
  genericSequence[0].touches[1].body_text = 'Just following up to see if you had any thoughts?';
  genericSequence[0].touches[1].evidence_used = [];
  await writeJson(path.join(campaignDir, '02-qualified.json'), qualifiedPayload);
  ...
  if (failures.length) {
    console.error(`Outreach v4 test failed with ${failures.length} defect(s):`);
    for (const failure of failures) console.error(`- ${failure}`);
    process.exitCode = 1;
  } else {
    console.log('Outreach v4 freshness, liveness, production-source, campaign, scheduling and ledger tests passed.');
  }
} finally {
